...
const GOAL_TYPE_LABELS: Record<string, { label: string; Icon: any }> = {
  EMERGENCY_FUND: { label: "Emergency Fund", Icon: ShieldCheck },
  ...
  return (
    <>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => setShowModal(true)} id="add-goal-btn">
          + Add New Goal
        </button>
      </div>
      ...
      <div className="page-body">
        {loading ? (
          <div className="text-muted" style={{ padding: "48px 0", textAlign: "center" }}>Loading goals…</div>
        ) : (
          <div className="two-col mb-6">
            {goals.map((goal) => {
              ...
              return (
                <div key={goal.id} className="card flex flex-col justify-between">
                    <div>
                      <div className="card-header mb-4">
                        <div>
                          <span className="badge gold flex items-center gap-1.5" style={{ display: "inline-flex" }}>
                            <GoalIcon size={12} />
                            <span>{meta.label}</span>
                          </span>
                          <h2 style={{ fontSize: 18, fontWeight: 700, marginTop: 6 }}>{goal.name}</h2>
                        </div>
                        <span className="badge blue">Priority #{goal.priority}</span>
                      </div>
                    ...
                    <div className="mb-4">
                      <div className="flex justify-between text-sm mb-2 font-semibold">
                        <span>Current: {formatZAR(current)}</span>
                        <span>Target: {target > 0 ? formatZAR(target) : "Formula"}</span>
                      </div>
                  ...
                  >
                    <div>
                      <div className="text-muted text-xs">Monthly Allocation</div>
                      <div className="font-bold text-gold">{formatZAR(Number(goal.monthlyContribution))}/mo</div>
                    </div>
                    <div style={{ textAlign: "right" }}>
                      <div className="text-muted text-xs">Projected Completion</div>
                      <div className="font-bold text-green">{goal.projection.projectedCompletionDate}</div>
                    </div>
                  </div>
                </div>
              );
