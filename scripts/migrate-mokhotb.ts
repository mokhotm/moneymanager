...
async function main() {
  console.log("==========================================================");
  console.log("🚀 Migrating User 'mokhotb' to Database...");
  console.log("==========================================================");
  ...
  if (!userB) {
    console.log("Creating new user 'mokhotb'...");
  ...
  allUsers.forEach((u) => {
    console.log(
      `- [${u.role.toUpperCase()}] ${u.username} (${u.email || "no-email"}) | Profile: ${
        u.profile?.fullName || "None"
      } | Tier: ${u.profile?.userSubscription?.tier?.name || "None"}`
    );
  });
  console.log("==========================================================");
}
