...
async function main() {
  ...
  for (const f of flows) {
    ...
    if (unmatched[key].samples.length < 2) {
  ...
  const sorted = Object.entries(unmatched)
    .filter(([_, data]) => data.total >= 100)
    .sort((a, b) => b[1].total - a[1].total);

  console.log(`\n=== TOP UNMATCHED MERCHANTS >= R100 (Count: ${sorted.length}) ===`);
  for (const [name, data] of sorted) {
    console.log(`- "${name}": R ${data.total.toFixed(2)} (${data.count} txs)`);
    data.samples.forEach((s) => console.log(`    ${s.date} | R ${s.amount}`));
  }
}
